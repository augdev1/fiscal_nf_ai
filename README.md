...
Escolha do FastAPI
Performance: adequado para APIs que processam múltiplos arquivos e operações de I/O intensivas.

Assincronicidade: uso de async/await para leitura de arquivos e chamadas externas (como Groq).

Documentação automática: OpenAPI/Swagger gerado automaticamente.

Validação: integração com Pydantic melhora a segurança dos dados.

Armazenamento de relatórios em disco (MVP)
Relatórios Excel e PDF são salvos em ./relatorios na versão inicial.

Essa abordagem simplifica o MVP e evita complexidade precoce com serviços de armazenamento em nuvem.

Em versões futuras, o uso de storage em cloud é um caminho natural para escalabilidade e durabilidade.

Aprendizados com Docker e volumes
Isolamento: ambiente consistente independentemente da máquina.

Persistência: volumes Docker (db_data para SQLite e ./relatorios:/app/relatorios) garantem que banco e relatórios sobrevivam a recriações de containers.

Orquestração: docker-compose facilita o gerenciamento do ciclo de vida da aplicação.

Depuração do problema de ICMS no frontend
Desafio: exibir corretamente os valores de ICMS e totais no front.

Causa: diferença entre os dados do backend e a forma como o JavaScript formatava/exibia.

Solução: inspeção das respostas da API, uso de toFixed(2) e formatação de moeda para garantir duas casas decimais e consistência visual com o backend.

📚 Documentação Extra
📓 Diário de Desenvolvimento e Docker

👨‍💻 Sobre o Desenvolvedor
O FiscalIA Pro foi concebido e desenvolvido em um estilo de pair programming com Inteligência Artificial, usando LLMs como parceiro de codificação.
Foi uma experiência de aprendizado autodidata, sem uso de vídeos tutoriais, focada em resolver problemas reais e construir um sistema funcional do zero.

Principais objetivos deste projeto:

Arquitetura Backend: desenhar e implementar uma API RESTful completa e bem estruturada.

Conteinerização com Docker: dominar Docker e Docker Compose para deploy e gerenciamento da aplicação.

Boas práticas de código: aplicar princípios de código limpo, modularidade e manutenibilidade em um projeto real.

text
>>>>>>> dc494f3fec26b4bb7a6b4d2279e8419e2cbf4814
